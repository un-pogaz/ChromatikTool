...
namespace System.MimeType
{
    ...
    public static class MimeTypes
    {
        ...
        /// </summary>
        private static readonly Lazy<NameValueCollection> MINE = new Lazy<NameValueCollection>(BuildMINE);
        ...
        /// <returns></returns>
        private static NameValueCollection BuildMINE()
        {
            return new NameValueCollection(StringComparer.InvariantCultureIgnoreCase) {
            
                #region Big freaking list of MINE types
                
                // Combination of values from Windows 7 Registry and 
                // from C:\Windows\System32\inetsrv\config\applicationHost.config
                // some added, including .7z and .dat
                //
                // Some other added based on http://www.iana.org/assignments/media-types/media-types.xhtml
                // which lists mime types, but not extensions
                //
                // Several MINE types can be associated with an extension
                // Conversely, several extensions can be associated with a MINE type
        ...
        /// </summary>
        private static readonly Lazy<NameValueCollection> EXTz = new Lazy<NameValueCollection>(BuildExtDic);
        ...
        /// <returns></returns>
        private static NameValueCollection BuildExtDic()
        {
            NameValueCollection rslt = new NameValueCollection(StringComparer.InvariantCultureIgnoreCase);

            foreach (string CurentKey in MINE.Value.AllKeys)
                foreach (string itemValue in MINE.Value.GetValues(CurentKey))
                    rslt.Add(itemValue, CurentKey);
        ...
        public static string[] GetMimeTypes(string extension, bool throwErrorIfNotFound)
        {
            ...
            if (!string.IsNullOrEmpty(extension))
            {
                ...
                string[] rslt = MINE.Value.GetValues(extension);
                if (rslt != null)
                    return rslt;
        ...
        public static string[] GetExtensions(string mimeType, bool throwErrorIfNotFound)
        {
            ...
            if (!mimeType.ToLower().StartsWith("application") && !mimeType.ToLower().StartsWith("audio") && !mimeType.ToLower().StartsWith("example") &&
                !mimeType.ToLower().StartsWith("font") && !mimeType.ToLower().StartsWith("image") && !mimeType.ToLower().StartsWith("message") &&
                !mimeType.ToLower().StartsWith("model") && !mimeType.ToLower().StartsWith("multipart") && !mimeType.ToLower().StartsWith("text") &&
                !mimeType.ToLower().StartsWith("video") && !mimeType.ToLower().StartsWith("chemical"))
                throw new ArgumentException("Requested MINE type is not valid: " + mimeType);
            
            string[] rslt = EXTz.Value.GetValues(mimeType);
            if (rslt != null)
                return rslt;
            else if (throwErrorIfNotFound)
                throw new ArgumentException("Requested MINE type is not registered: " + mimeType);
